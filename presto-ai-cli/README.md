...
```bash
presto-ai config set presto_https true
presto-ai config set presto_insecure true  # Skip SSL verification
```

**For OAuth2/SSO Protected Presto (oauth2-proxy):**

Option A - Automated browser login (requires selenium):
```bash
pip install selenium
presto-ai login  # Opens browser, captures cookie after you authenticate
```

Option B - Manual cookie entry:
```bash
presto-ai login --method manual  # Guides you through copying cookie from DevTools
```

Option C - Direct cookie configuration:
```bash
# Get cookie from browser DevTools: Application → Cookies → _oauth2_proxy
presto-ai config set presto_cookie 'your-oauth2-proxy-cookie-value'
```
...
- Your OAuth cookie may have expired (typically 24h)
- Re-authenticate: `presto-ai login`

**SSL Certificate errors:**
```bash
# Enable insecure mode for self-signed certs
presto-ai config set presto_insecure true
```

**Cookie not working:**
1. Make sure you're copying the full `_oauth2_proxy` cookie value
2. The cookie value contains `|` characters - copy the entire string
3. Try the manual login flow: `presto-ai login --method manual`
